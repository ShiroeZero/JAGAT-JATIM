...
def main():
    ...
    for case in cases:
        ...
        breakdown = case.get("priority_breakdown") or {}
        score = case.get("priority_score")
        if score is None:
            errors.append(f"{cid} missing priority_score")
        else:
            expected_score = sum(int(breakdown.get(k) or 0) for k in ("severity", "escalation", "spread", "current_activity"))
            if int(score) != expected_score:
                errors.append(f"{cid} priority_score mismatch: stored={score}, expected={expected_score}")
            expected_priority = priority_for_score(score, breakdown.get("severity", 0))
            actual_priority = str(case.get("priority") or "low").lower()
            if actual_priority != expected_priority:
                errors.append(f"{cid} priority mismatch: stored={actual_priority}, expected={expected_priority}")
